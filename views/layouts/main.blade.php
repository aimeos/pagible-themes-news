...
<html class="no-js" data-theme="light" lang="{{ cms($page, 'lang') }}" dir="{{ in_array(cms($page, 'lang'), ['ar', 'az', 'dv', 'fa', 'he', 'ku', 'ur']) ? 'rtl' : 'ltr' }}">
    <head>
        ...
        <meta name="theme-color" content="{{ cms($page, 'config.theme.data.--pico-background-color') ?: '#F8FBFF' }}">
    ...
    <body class="theme-news type-{{ cms($page, 'type') ?: 'page' }}">
        ...
        <footer class="bottom">
            <div class="container">
                @foreach($page->ancestorsAndSelf->reverse() as $navItem)
                    @if($fileId = cms($navItem, 'config.logo.data.file.id'))
                        <img class="brand" src="{{ cmsasset($navItem, cmsfile($navItem, $fileId)) }}" alt="{{ config('app.name') }}">
                        @break
                    @endif
                @endforeach
